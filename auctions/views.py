...
from auctions.forms import NewAuctionForm, NewBidForm
...
def create_bid(request, auction_id):
    if request.method == "POST":
        ...
        if bid_form.is_valid():
            ...
            return redirect(reverse("auction_index", kwargs={
                "auction_id": auction_id,
            }), {
                "errors": bid_form.errors
            })
    else:
        return HttpResponseBadRequest("Invalid method used")
...
def category_auctions(request, category_id):
    return render(request, "auctions/index.html", {
        "auctions": Auction.objects.filter(categories__id=category_id, is_closed=False)
    })